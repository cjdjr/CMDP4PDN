...
class MLP2HCritic(nn.Module):
    def __init__(self, input_shape, output_shape, args):
        super(MLP2HCritic, self).__init__()
        self.args = args
        ...
        self.cost_fc2 = nn.Linear(args.hid_size, args.hid_size)
        self.cost_fc3 = nn.Linear(args.hid_size, output_shape)
    ...
    def forward(self, inputs, hidden_state):
        ...
        emb = self.hid_activation(emb)
        h = self.hid_activation(self.fc2(emb))
        v = self.fc3(h)
        h = self.hid_activation(self.cost_fc2(emb))
        c = self.cost_fc3(h)
        return v, c, h